...
const Container = ({ data }) => {
    return (
        <div className="">
            ...
            >
                <h2 className="sm:text-4xl text-xl font-extrabold mb-8 w-full mx-auto text-center text-white p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    {data.classement.map((item, index) => (
                        <Link
                            href={`${process.env.NEXT_PUBLIC_SITE_URL}/class/${slugify(item.type)}/${slugify(item.title)}`}
                            key={index}
                        ...
                        >
                            <div className="flex justify-center sm:p-8 p-4">
                                <img src={item.logo ? item.logo : item.title} className={`w-[80px] h-[80px] p-4`} />
                            </div>
