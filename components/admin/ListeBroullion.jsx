...
const ListeBroullion = ({ produit }) => {
    ...
    const fetchOffre = async () => {
        try {
            setLoading(true)
            const response = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/broullion`)
    ...
    const handleBroullion = async (id) => {
        try {
            const response = await axios.put(`${process.env.NEXT_PUBLIC_SITE_URL}/api/broullion/${id}?status=broullion`)
            fetchOffre()
            alert(response.data.message)
            // window.location.reload()
        } catch (error) {
    ...
    return (
        <div className='text-black'>
            <div className="flex items-center justify-between mb-5">
                <h1 className='flex justify-center text-xl font-medium'>Liste des offres mis en broullion</h1>
            ...
            <div className="space-y-5">
                {
                    filteredData.length == 0 ?
                        ...
                        filteredData.map((offre, index) =>
                            <Offre key={index} data={offre} onClickBro={() => handleBroullion(offre.id)} onClickDel={() => handleDelete(offre.id)} />
                        )
                }
            </div>
        </div>
...
export default ListeBroullion

const Offre = ({ data, onClickDel, onClickBro }) => {
    return (
        <div className="xs:flex p-3 rounded-md justify-between items-center border-gray-200 border-2 hover:bg-gray-200">
            ...
            <div className="">
                <div className="flex space-x-1">
                    ...
                    <button onClick={onClickBro} className='text-blue-500'>Publier</button>
