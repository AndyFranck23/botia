import { cookies } from 'next/headers'
import { redirect } from 'next/navigation'
import jwt from 'jsonwebtoken'
import HeaderAdmin from '@/components/admin/HeaderAdmin'
import { MenuAdmin } from '@/components/admin/MenuAdmin'
import { UserProvider } from '../../components/admin/context/UserContext'
import { queryDB } from '@/lib/db'

const LOGIN_ADMIN = process.env.LOGIN_ADMIN

export async function generateMetadata() {
    return {
        robots: "noindex, nofollow"
    }
}


export default async function Layout({ children }) {
    const cookieStore = await cookies()
    const token = cookieStore.get('authToken')?.value

    if (!token) {
        redirect('/login')
    }

    try {
        const decoded = jwt.verify(token, process.env.JWT_SECRET)

        const user = await queryDB(
            'SELECT * FROM users WHERE email = ?',
            [decoded.email]
        )

        if (user.length === 0) {
            redirect('/login')
        }

        let out = false
        if (user[0].email == LOGIN_ADMIN || user[0].email == 'ironmanandy23@gmail.com')
            out = true

        // const isAdmin = decoded.email === process.env.LOGIN_ADMIN

        return (
            <UserProvider user={user[0]}>
                <HeaderAdmin userdata={user[0]} userType={out} />
                <MenuAdmin className={'sm:block hidden'} userType={out} />
                <div className="sm:ml-48 text-white text-sm sm:text-md h-screen pt-10">
                    <div className="w-full p-6 shadow-md rounded-lg md:flex justify-center">
                        <div className="md:w-[600px] lg:w-[900px] ">
                            {children}
                        </div>
                    </div>
                </div>
            </UserProvider>
        )

    } catch (error) {
        console.error('Token verification failed:', error)
        redirect('/login')
    }
}
